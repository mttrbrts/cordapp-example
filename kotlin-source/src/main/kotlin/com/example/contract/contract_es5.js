...
function unwrap(doc) {
  ...
  if ("state" in doc && !("$class" in doc)) {
    if (doc.state == "COMMITTED") return JSON.parse(doc.currentValue);else return null; // Not sure if we will need something more fancy for un-committed data
  } // Leave as-is
  else return doc;
}
...
function concat(r1, r2) {
  ...
  for (var key2 in r2) {
    if (!(key2 in r1)) result[key2] = r2[key2];
  }
...
function contains(v, b) {
  for (var i = 0; i < b.length; i++) {
    if (equal(v, toLeft(b[i]))) return true;
  }
...
function compare(v1, v2) {
  ...
  if (a1 == "[object Array]") {
    ...
    v2 = v2.slice();
    /* So we do the sort/compare on a clone of the original array */
  ...
  if (t1 == "object") {
    ...
    for (var i = 0; i < fields1.length; i++) {
      if (!(fields1[i] in v2)) return -1;
      var fc = compare(v1[fields1[i]], v2[fields1[i]]);
      if (fc != 0) return fc;
    }

    for (var i = 0; i < fields2.length; i++) {
      if (!(fields2[i] in v1)) return +1;
    }
...
function compareOfMultipleCriterias(scl) {
  return function (a, b) {
    ...
    for (var i = 0; i < scl.length; i++) {
      ...
      if ("asc" in sc) {
        current_compare = compare(deref(a, sc['asc']), deref(b, sc['asc']));
      } else if ("desc" in sc) {
        current_compare = -compare(deref(a, sc['asc']), deref(b, sc['asc']));
      }
...
function project(r1, p2) {
  ...
  for (var key1 in r1) {
    if (!(p2.indexOf(key1) == -1)) result[key1] = r1[key1];
  }
...
function toStringQ(v, quote) {
  ...
  if (t == "string") return quote + v + quote;
...
function cast(brands, v) {
  ...
  mustBeArray(brands);
  if ("$class" in v) return enhanced_cast(brands, v);
  var type = v.type;
  mustBeArray(type);

  if (brands.length == 1 && brands[0] == "Any") {
    /* cast to top of inheritance is built-in */
    return left(v);
  }

  brands: for (var i in brands) {
    var b = brands[i];

    for (var j in type) {
      var t = type[j];
      if (equal(t, b) || sub_brand(t, b)) continue brands;
    }
...
function enhanced_cast(brands, v) {
  var type = v.$class;
  if (brands.length != 1) throw "Can't handle multiple brands yet";
  var brand = brands[0];
...
function unbrand(v) {
  if (_typeof(v) === "object") if ("$class" in v) {
...
function either(v) {
  ...
  if (_typeof(v) === "object") return !("right" in v);
  return true;
}

function toLeft(v) {
  if (_typeof(v) === "object") {
    if ("left" in v) return v.left;
    if ("$value" in v) return v.$value;
    if (looksLikeRelationship(v)) return v["key"];
  }
...
function toRight(v) {
  if (v === null) return null;
  if (_typeof(v) === "object" && "right" in v) return v.right;
  return undefined;
}

function deref(receiver, member) {
  if (_typeof(receiver) === "object" && member in receiver) {
    ...
    if (_typeof(ans) === "object" && looksLikeRelationship(ans)) ans = left(ans["key"]);
    if ("$class" in receiver && _typeof(ans) === "object" && !("left" in ans) && !Array.isArray(ans)) ans = left(ans);
    return ans;
  }
...
function looksLikeRelationship(v) {
  ...
  for (var member in v) {
    if (member == "key") hasKey = true;else if (member == "type") hasType = true;else if (member == "$class" && v["$class"] == "com.ibm.ia.model.Relationship") continue;else return false;
  }
...
function natMin(v1, v2) {
  return {
    "nat": Math.min(v1.nat, v2.nat)
  };
}

function natMax(v1, v2) {
  return {
    "nat": Math.max(v1.nat, v2.nat)
  };
}
...
function natLog2(v) {
  return {
    "nat": Math.floor(Math.log2(v.nat))
  }; // Default Z.log2 is log_inf, biggest integer lower than log2
}

function natSqrt(v) {
  return {
    "nat": Math.floor(Math.sqrt(v.nat))
  }; // See Z.sqrt biggest integer lower than sqrt
}
...
function natMinApply(b) {
  ...
  return {
    "nat": Math.min.apply(Math, numbers)
  };
}
...
function natMaxApply(b) {
  ...
  return {
    "nat": Math.max.apply(Math, numbers)
  };
}
...
function mustBeUnit(unit) {
  if (unit === DAY || unit === MONTH || unit === QUARTER || unit === YEAR) return;
...
var orgXaccordprojectXsampleXcordaXiouXIOUContract =
/*#__PURE__*/
function () {
  function orgXaccordprojectXsampleXcordaXiouXIOUContract() {
    _classCallCheck(this, orgXaccordprojectXsampleXcordaXiouXIOUContract);
  }

  _createClass(orgXaccordprojectXsampleXcordaXiouXIOUContract, [{
    key: "main",
    value: function main(context) {
      ...
      if (either(cast(["org.accordproject.sample.corda.iou.IOURequest"], vX$match0))) {
        var vX$case0 = null;
        vX$case0 = toLeft(cast(["org.accordproject.sample.corda.iou.IOURequest"], vX$match0));
        res1 = {
          "left": {
            "$main": vX$case0
          }
        ...
        var vX$case1 = null;
        vX$case1 = toRight(cast(["org.accordproject.sample.corda.iou.IOURequest"], vX$match0));
        res1 = {
          "right": null
        };
      }
      ...
      if (either(res1)) {
        ...
        if (!!equal(deref(unbrand(vcontract), "lender"), deref(unbrand(vcontract), "borrower"))) {
          t3 = {
          ...
          if (!(floatOfNat(vp1) > 0.0)) {
            ...
            t2 = {
              "left": concat(concat({
                "response": brand(["org.accordproject.cicero.runtime.Response"], {})
              }, {
                "state": vlstate
              }), {
                "emit": vlemit
        ...
        res4 = {
          "right": {
            ...
            "data": {
              "message": "DefaultMatch Error at lib/logic.ergo 19:0-29:1 ''"
            }
          }
        };
      }
    ...
    value: function init(context) {
      ...
      var vlstate_0 = vstate;
      var vlemit = vemit;
      var vlstate = brand(["org.accordproject.cicero.contract.AccordContractState"], {
        "stateId": "1"
      });
      return {
        "left": concat(concat({
  ...
  return orgXaccordprojectXsampleXcordaXiouXIOUContract;
}();
